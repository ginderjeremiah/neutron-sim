...
const double MAXTIME = 3.8; //time to simulate neutron (sim time not real time)
const double maxSpinStep = 1 * pow(10, -9);
const double minSpinStep = 1 * pow(10, -9);
const double eulerStep = 1 * pow(10, -9);
...
int main(int argCount, char** argv)
{
	...
	while (((GEOMETRY_MODE == 1) ? zetaCoordinate(tempPosition2) : tempPosition2[2] + 1.5) > 0 && timeGoal < MAXTIME)
	{
		...
		if (numberOfSteps2 == 0)
		{
			...
			*/
			magFieldCalc(tempPosition2, startMag);
			fieldStrength = magnitude(startMag);
			for (int l = 0; l < 3; ++l)
			{
				spin[l] = startMag[l] * (HBAR / 2) / fieldStrength;
			}
		}
		...
		while((t < timeGoal || numberOfSteps2 == 0) && ((GEOMETRY_MODE == 1) ? zetaCoordinate(tempPosition2) : tempPosition2[2] + 1.5) > 0)
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				spin[i] += 1.0/2 * eulerStep * GYRO_RATIO * spinCrossField[i];
			}
			
			/*
			if (INTEGRATION_MODE == 1)
			{
				spinMag = magnitude(spin);
				
				for (int i = 0; i < 3; ++i)
				{
					spin[i] = spin[i] / spinMag * (HBAR / 2);
				}
			}
			*/
			...
			for (int i = 0; i < 3; ++i)
			{
				magField[i] *= -1.0/2 * eulerStep * GYRO_RATIO;
			}
			
			determinant = 1.0 + pow(magnitude(magField), 2);
			...
			else if (OUTPUT_MODE)
			{
				...
				if (t >= outputTimeGoal)
				{
					...
					outFile << t << " " << tempPosition2[0] << " "  << tempPosition2[1] << " "  <<  1000*(tempPosition2[2] + 1.5) << " " << abs(polarization - previousPolarization) << " " << 1.0 - polarization << " " << magnitude(magField) << " " << dotProduct(spin, startMag) / magnitude(spin) / magnitude(startMag) << endl;
