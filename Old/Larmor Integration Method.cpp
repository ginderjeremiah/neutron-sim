...
const double MAXTIME = 1; //time to simulate neutron (sim time not real time)
const double maxSpinStep = 1 * pow(10, -7);
const double minSpinStep = 1 * pow(10, -9);
const double dynamicsStep = 1 * pow(10, -6);
const double minLarmorStep = 1 * pow(10, -9);
const double maxControl = 35.0;
const double minControl = -1;
const int INTEGRATION_MODE = 0; //set 0 for normal, 1 for spin normalization every step
...
int main(int argCount, char** argv)
{
	...
	while (((GEOMETRY_MODE == 1) ? zetaCoordinate(tempPosition2) : tempPosition2[2] + 1.5) > 0 && timeGoal < MAXTIME)
	{
		...
		{
			control = (magnitude(magField) - holdField) / holdField;
			if (control > maxControl || control < minControl)
			{
				larmorSteps++;
				if (larmorStep > timeGoal - t)
				{
					larmorStep = timeGoal - t;
				}
				else
				{
					larmorStep = minLarmorStep;
				}
				
				for (int i = 0; i < 3; ++i)
				{
					tempPosition[i] = tempPosition2[i];
				}
				...
				for (int i = 0; i < 5; i++)
				{
					...
					larmorFreq = -GYRO_RATIO * magnitude(magField);
					angleRotated = larmorFreq * larmorStep;
					
					for (int j = 0; j < 3; ++j)
					{
						increment[i][j] = magField[j];
					}
					increment[i][3] = angleRotated;
					
					for (int j = 0; j < 3; ++j)
					{
						tempPosition[j] = position[j] + tempVelocity[j] * larmorStep * coefficients[i][0];
					}
				}
				...
				for (int j = 0; j < 3; ++j)
				{
					increment[5][j] = magField[j];
				}
				increment[5][3] = angleRotated;
				...
				for (int j = 0; j < 6; j++)
				{
					if (order4[j] != 0)
					{
						createRotationMatrix(rotationMatrix, increment[j], increment[j][3] * order4[j]);
						...
						for (int r = 0; r < 3; r++)
						{
							...
							for (int b = 0; b < 3; b++)
							{
								tempSpin[r] += tempSpin2[b] * rotationMatrix[r][b];
							}
						}
						
						for (int k = 0; k < 3; ++k)
						{
							tempSpin2[k] = tempSpin[k];
						}
					}
				}
				...
				for (int j = 0; j < 6; j++)
				{
					if (order5[j] != 0)
					{
						createRotationMatrix(rotationMatrix, increment[j], increment[j][3] * order5[j]);
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				if (control > maxControl || control < minControl)
				{
					tempPosition2[i] += larmorStep * tempVelocity[i];
				}
				else
				{
			...
			else if (OUTPUT_MODE)
			{
				...
				if (t >= outputTimeGoal)
				{
					...
					magFieldCalc(tempPosition2, magField);
					magPol = dotProduct(mag0, magField) / magnitude(mag0) / magnitude(magField);
					polarization = dotProduct(spin, magField) / magnitude(spin) / magnitude(magField);
					outFile << t << " " << tempPosition2[0] << " "  << tempPosition2[1] << " "  <<  1000*(tempPosition2[2] + 1.5) << " " << (1.0 - (magPol - prevMagPol)) / (1.0 - polarization) << " " << 1.0 - polarization << " " << magnitude(magField) << " " << larmorSteps << endl;
